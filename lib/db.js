...
export async function createCollege(name, location) {
	const sql = "INSERT INTO colleges (name, location) VALUES (?, ?)";
	return await query(sql, [name, location]);
}
...
export async function getApplicationsByStudent(studentId) {
	const sql = `
    SELECT ja.*, j.title, j.location, j.deadline, j.company_name
